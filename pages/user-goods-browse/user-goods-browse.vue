...
<scroll-view scroll-y="true" class="scroll-box" @scrolltolower="scroll_lower" lower-threshold="30">
  <view class="list-item" v-if="data_list.length > 0">
    <view v-for="(item, index) in data_list" :key="index" class="goods-item oh bg-white spacing-mb">
      ...
      <button class="submit-cancel" type="default" size="mini" @tap="delete_event" :data-value="item.id" :data-index="index" hover-class="none">删除</button>
    </view>
  </view>
...
export default {
  ...
  onShow() {
    wx.setNavigationBarTitle({
      title: app.globalData.data.common_pages_title.user_goods_browse
    });
    this.init();
  },
